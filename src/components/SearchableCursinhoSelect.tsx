...
import CURSINHOS from '../data/cursinhos.json';
...
export default function SearchableCursinhoSelect({ value, onSelect, placeholder, id }: Props) {
  ...
  const filtered = CURSINHOS.filter((f) => f.toLowerCase().includes(query.toLowerCase())).slice(0, 100);
  ...
  return (
    <div className="relative" ref={rootRef}>
      <input
        ...
        placeholder={isOtherMode ? 'Digite o nome do cursinho' : placeholder}
          ...
          <ul className="p-2">
            ...
            <li className="border-t border-white/5 mt-2 pt-2">
              <button type="button" onClick={handleOutra} className="w-full text-left px-3 py-2 text-sm text-amber-300 hover:bg-white/5 rounded-md">
                Meu cursinho não está na lista
              </button>
            </li>
          </ul>
        </div>
